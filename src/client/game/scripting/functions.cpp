...
namespace scripting
{
	namespace
	{
		int find_function_index(const std::string& name, const bool prefer_global)
		{
			...
			auto first = xsk::gsc::iw7::resolver::function_id;
			auto second = xsk::gsc::iw7::resolver::method_id;
			if (!prefer_global)
			{
	...
	{
		return xsk::gsc::iw7::resolver::token_name(static_cast<std::uint16_t>(id));
	}

	unsigned int find_token_id(const std::string& name)
	{
		const auto result = xsk::gsc::iw7::resolver::token_id(name);
		if (result)
		{
			return result;
		}
