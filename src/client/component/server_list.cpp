...
namespace server_list
{
	namespace
	{
		...
		{
#ifdef DEBUG
			console::info("[SERVER_LIST] Refreshing!\n");
#endif
			{
		...
		int ui_feeder_count()
		{
			...
			console::info("Writing Server Count %d\n", count);
		...
		const char *ui_feeder_item_text(const int index, const int column)
		{
			...
			console::info("[FEEDER] i >= serverCount? %d\n", i >= servers.size());
			if (i >= servers.size())
			{
				return "";
			}
	...
	bool get_master_server(game::netadr_s &address)
	{
		return game::NET_StringToAdr(utils::string::va("%s:%s",
													   master_server_ip->current.string, master_server_port->current.string),
									 &address);
	}
	...
	void handle_info_response(const game::netadr_s &address, const utils::info_string &info)
	{
		...
		server.in_game = 1;
		console::info("Should be inserting %s\n", server.host_name.data());
	...
	public:
		void post_unpack() override
		{
			scheduler::once([]()
					...
					master_server_ip = game::Dvar_RegisterString("masterServerIP", "server.alterware.dev", game::DVAR_FLAG_NONE, "IP of the destination master server to connect to");
					master_server_port = game::Dvar_RegisterString("masterServerPort", "20810", game::DVAR_FLAG_NONE, "Port of the destination master server to connect to"); },
			...
			// do feeder stuff
			utils::hook::jump(/*0x28E117_b*/ 0x69E0CF_b, utils::hook::assemble([](utils::hook::assembler &a)
																			   {
					a.mov(ecx, eax);
					...
					a.jmp(/*0x28E12B_b*/0x69E0E3_b); }),
							  true);

			utils::hook::jump(/*0x28E331_b*/ 0x69E2E8_b, utils::hook::assemble([](utils::hook::assembler &a)
																			   {
					a.push(rax);
					a.pushad64();
					a.mov(rcx, r9);  // index
