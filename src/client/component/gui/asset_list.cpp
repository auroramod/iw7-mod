...
namespace gui::asset_list
{
	namespace
	{
		...
		void draw_table_row(game::XAssetType type, const game::XAsset* asset, const game::DB_AssetEntry* entry, bool should_add_view_btn)
		{
			auto asset_name = game::DB_GetXAssetName(asset);
			const auto is_default = entry->m_zoneIndex == 0;

			if (asset_name[0] == '\0' || disabled_zones[type][entry->m_zoneIndex] || default_only[type] && !is_default)
			{
				return;
			}
			...
			{
				if (!utils::string::strstr_lower(asset->header.localize->value, assets_value_filter[type].data()))
				{
					return;
				}
			}
			...
			{
				ImGui::TableSetColumnIndex(col_index++);
				if (entry->m_zoneIndex > 0)
				{
					ImGui::Text( "%s", game::DB_Zones_GetZoneNameFromIndex( static_cast<int>(entry->m_zoneIndex) ));
				}
				else
				{
					ImGui::Text("default");
			...
			if (type == game::ASSET_TYPE_LOCALIZE_ENTRY)
			{
				...
				if (ImGui::Button(asset->header.localize->value))
				{
					copy_to_clipboard(asset->header.localize->value);
				}
			}
		}
		...
		void draw_asset_type_list()
		{
			...
			{
				for (auto i = 0; i < 373; ++i)
				{
					if (!game::DB_Zones_IsValidZoneIndex(i))
					{
						break;
					}
					
					if (const auto zone_name = game::DB_Zones_GetZoneNameFromIndex(i); ImGui::Button(zone_name))
					{
						copy_to_clipboard(zone_name);
					}
				}
			...
			for (auto i = 0; i < game::XAssetType::ASSET_TYPE_COUNT; i++)
			{
				...
				if (asset_type_filter.empty() || utils::string::strstr_lower(name, asset_type_filter.data()))
				{
		...
		void draw_asset_list_filter(const game::XAssetType type)
		{
			...
			{
				for (auto zone = 0; zone < 373; ++zone)
				{
					const auto zone_name = game::DB_Zones_GetZoneNameFromIndex(zone);
		...
		void draw_asset_list_entries(const game::XAssetType type)
		{
			...
			{
				game::DB_EnumXAssets(type, [&](const game::XAssetHeader header)
				{
					// the asset is for easy use
					const auto asset = game::XAsset{ type, header };
					
					// grab the entry so we can get the zone index
					...
					asset_count[type]++;
					draw_table_row(type, &asset, entry, should_add_view_btn);
				});
	...
	public:
		void post_unpack() override
		{
			...
			register_menu("asset_list", "Asset List", render_window);
		}
	};
}
