...
namespace mapents
{
	namespace
	{
		...
		{
			auto mapents_name = utils::string::va("%s.ents", **reinterpret_cast<const char***>(0x145D5F9D8));
		...
		const char* cm_entity_string_stub()
		{
			...
			else
			{
				...
				ents = utils::hook::invoke<const char*>(0x140B7B0C0);
			}
		...
		void cm_unload_stub(void* clip_map)
		{
			entity_string.clear();
			raw_ents.clear();
			utils::hook::invoke<void>(0x140B7B070, clip_map);
		}
	}
	...
	public:
		...
		{
			utils::hook::call(0x140B1EC04, cm_entity_string_stub);
			utils::hook::call(0x1403B7B17, cm_unload_stub);
		}
	};
}
